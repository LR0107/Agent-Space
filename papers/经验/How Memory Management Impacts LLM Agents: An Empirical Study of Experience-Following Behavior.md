...
注：一次任务完成以后，得到一条：(qt​,et​)。其中 qt​ 是当前任务输入，et​ 是 Agent 的执行结果/trajectory。

然后 Evaluator 判断这次执行质量。如果通过，就把这对 query–execution直接作为一条 Memory 加入 Memory Bank；没有专门再做总结、抽取、反思、提炼。

新任务来了以后，从整个当前 Memory Bank中检索最相似的 Top-K 条经验。

论文中的 SR / ACC 是 Agent 在整个测试任务流上的总体性能指标。例如 RegAgent 一共运行约 4000 个任务，每个任务判断成功/失败，最终统计整体 **Success Rate**
...
3.没有系统统计 token 开销、API 成本、检索延迟。

论文统计的资源开销主要是 **Memory Size**，也就是最终 Memory Bank 保留多少条记录。
